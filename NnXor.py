...
def nnLoadNet():
    ...
    global error
    global count
    flnm =  filedialog.askopenfilename(initialdir = "",filetypes = (("net files","*.net"),("all files","*.*")))
...
def nnSaveNet():
    global net
    flnm = asksaveasfile(initialfile = 'Untitled.txt',
        defaultextension=".net",filetypes=[("All Files","*.*"),("Text Documents","*.txt")])
    net.save_weights(flnm.name)
#.............................................
def nnLoadData():
    global net
    # flnm = filedialog.askopenfilename(("data files", "*.dat"),("all files", "*.*"))
    flnm = filedialog.askopenfilename(initialdir = "",filetypes = (("data files","*.dat"),("all files","*.*")))    
    net.load_data(flnm)
...
btnSvXorDat = tk.Button(place3, text="Sav Xor Dt", bg="lightgray", command=nnSaveXorData)
